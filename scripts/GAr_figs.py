...
def spatialEmissFig(data,xlon,ylat,legend,cmap,borderShape,folder,pol,emissType,borderShape2=None):
    ...
    heatmap = ax.pcolor(xlon,ylat,data,cmap=cmap,norm=colors.LogNorm())
    cbar = fig.colorbar(heatmap,fraction=0.04, pad=0.02,
                        #extend='both',
                        #ticks=bounds,
                        spacing='uniform',
    ...
    if borderShape2!=None:
        br = gpd.read_file(borderShape)
        br.boundary.plot(edgecolor='gray',linewidth=0.5,ax=ax)
        br2 = gpd.read_file(borderShape2)
        br2.boundary.plot(edgecolor='black',linewidth=0.5,ax=ax)
    else:
        br = gpd.read_file(borderShape)
        br.boundary.plot(edgecolor='black',linewidth=0.5,ax=ax)
